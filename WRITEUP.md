...
**The mechanic.** Osero owes Sky SSR plus 20 bps, but only on the slice of its deployed USDS that someone downstream actually borrowed. In a pooled market like SparkLend, capital is fungible, so Osero's borrowed slice is its position times the reserve's utilization (variable debt divided by aToken supply). Idle liquidity in the pool costs nothing and earns nothing.
